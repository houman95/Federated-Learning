...
for seed in seeds_for_avg:
    ...
    # Initialize the transmission probability
    tx_prob = 0

    # Define the tao vector provided
    tao = [0.0,
 0.16865627944353181,
 0.8820699950593773,
 0.6455434571072044,
 ...
 0.4281564457227726]
    ...
    # Define the optimal user dictionary
    optimal_user_per_timeframe = {1: 1, 2: 7, 3: 6, 4: 6, 5: 5, 6: 5, 7: 6, 8: 4, 9: 4, 10: 3, 11: 4, 12: 4, 13: 4, 14: 3, 15: 4}
    ...
    for timeframe in range(num_timeframes):
        ...
        user_gradients.sort(key=lambda x: x[0])

        if timeframe == 0:
          # (a) In the first timeframe, only let user 1 transmit
          num_active_users = 1
          tx_prob = 1
          print("*** Only User 1 Active ***")
        else:
          # (b) For the rest of the timeframes
          # Calculate the local gradient over the previous global gradient’s ratio          
          active_users = [user_id for user_id, grad_norm, _ in user_gradients if (grad_norm / (global_grad_mag[seed_count - 2, timeframe - 1, user_id] + epsilon)) > tao[timeframe]]
          if active_users:
            U_A = optimal_user_per_timeframe[timeframe + 1]
            x_prob = 1 / U_A
            num_active_users = len(active_users)
            print(f"*** {num_active_users} Active Users ***")
          else:
            num_active_users = 10
            x_prob = 0
