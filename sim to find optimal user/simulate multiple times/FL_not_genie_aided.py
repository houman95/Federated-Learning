...
# Accuracy distributions without num_active_users
accuracy_distributions = {
    run: {
        seed_index: {
            timeframe: None for timeframe in range(num_timeframes)  # Store only a single accuracy value per timeframe
        } for seed_index in range(len(seeds_for_avg))
    } for run in range(num_runs)
}

# Best accuracy distributions remain the same
best_accuracy_distributions = {
    run: {
        seed_index: {timeframe: None for timeframe in range(num_timeframes)}  # Store the best accuracy for each timeframe
        for seed_index in range(len(seeds_for_avg))
    } for run in range(num_runs)
}

# Correctly received packets statistics without num_active_users
correctly_received_packets_stats = {
    run: {
        seed_index: {
            timeframe: {'mean': None, 'variance': None}  # Store mean and variance directly per timeframe
